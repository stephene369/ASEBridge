...
export const PostForm = ({ post , action }: PostFormProps ) => {
  ...
  async function onSubmit(values: z.infer<typeof PostValidation>) {
    if (post && action ==='Update' ){
      console.log(values)
      const updatePost = await UpdatePost({ ...values , 
        postId: post.$id, 
      ...
      <form onSubmit={form.handleSubmit(onSubmit)}
            ...
            <FormItem>
              ...
              <FormControl>
                <Input type="text"
                  ...
                  placeholder="Art, Coding, Learning, etc"
                />
              </FormControl>
              <FormMessage className="shad-form_message" />
            </FormItem>
        ...
        <div className="flex gap-4 items-center justify-end">
          ...
          <Button type="submit" className="shad-button_primary whitespace-nowrap" 
            disabled={isLoadingCreating || isLoadingUpdate || isLoadingDelete } >
            { isLoadingCreating || isLoadingUpdate && 'Loading...' } {action} Post
            </Button>
        </div>
      </form>
    </Form>
